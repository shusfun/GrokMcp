...
export const wailsClient: Client = {
  ...
  readResult: (id, request, turn, offset) => call("ReadResult", id, request, turn ?? "", offset ?? 0, 16384),
  events: async (id) => (await call<BoundaryEvent[] | null>("Events", id)) ?? [],
  statusBar: () => call("StatusBar"),
  setView: (id, view) => call("SetView", id, view),
  cancelTurn: (id, turnId, requestId) => call("CancelTurnBound", id, turnId ?? "", requestId ?? ""),
  continueJob: (id) => call("Continue", id),
  planDecide: (id, decide, notes, binding) => call("PlanDecideBound", id, decide, notes ?? "", binding?.request_id ?? "", binding?.plan_turn_id ?? "", binding?.plan_version ?? 0, binding?.approval_id ?? ""),
  ...
  mcpConfig: () => call("MCPConfig"),
  mcpStatus: () => call("MCPStatus"),
  openCCSwitchMCPImport: () => call("OpenCCSwitchMCPImport"),
  openCCSwitchApp: () => call("OpenCCSwitchApp"),
  addMCPToCodex: () => call("AddMCPToCodex"),
